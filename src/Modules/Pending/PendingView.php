...
<?php
  ...
  {
    if ($title === 'Contrato') {
      return $this->renderPendingContractRecordSection($record, $sectionId);
    }
  ...
  /** @param array<string,mixed> $record */
  private function renderPendingContractRecordSection(array $record, string $sectionId): string
  {
    $html = '<section class="scm-case-history scm-contract-detail" id="' . esc_attr($sectionId) . '"><h4>Contrato</h4>';
    if (empty($record)) {
      return $html . '<p class="scm-case-history-empty">Sin datos.</p></section>';
    }

    $contractId = $this->pendingFirstRecordValue($record, ['contrato', 'id_contrato', '_ID']);
    ...
    $html .= '<div class="scm-contract-title"><span class="material-symbols-outlined">description</span><div><strong>Contrato de Arrendamiento</strong><small>' . esc_html($this->pendingContractMetaLine($propertyWeb, $propertySimi)) . '</small></div></div>';
    ...
    $html .= $this->renderPendingContractParty('Propietario', 'Titular', $record, ['propietario', 'nombre_propietario'], ['correo_propietario', 'email_propietario'], ['celular_propietario', 'telefono_propietario']);
    $html .= $this->renderPendingContractParty('Arrendatario', 'Activo', $record, ['arrendatario', 'nombre_arrendatario'], ['correo_arrendatario', 'email_arrendatario'], ['celular_arrendatario', 'telefono_arrendatario']);
    $html .= '</div>';
    ...
    $html .= $this->renderPendingContractMetric('Canon mensual', $this->pendingContractMoney($this->pendingFirstRecordValue($record, ['valor_canon', 'canon', 'canon_arrendamiento'])), '');
    $html .= $this->renderPendingContractMetric('Administracion', $this->pendingContractMoney($this->pendingFirstRecordValue($record, ['valor_administracion', 'administracion', 'precio_admin'])), '');
    $html .= $this->renderPendingContractMetric('Tasa admin', $this->pendingContractPercent($this->pendingFirstRecordValue($record, ['tasa_administracion', 'porcentaje_incremento_admin'])), '');
    $html .= $this->renderPendingContractMetric('Derechos inmob.', $this->pendingContractPercent($this->pendingFirstRecordValue($record, ['derechos_inmobiliarios'])), '');
  ...
  private function pendingFirstRecordValue(array $record, array $keys): string
  {
    foreach ($keys as $key) {
  ...
  private function pendingContractMetaLine(string $propertyWeb, string $propertySimi): string
  {
    $parts = [];
    if ($propertyWeb !== '') {
  ...
  private function pendingContractDisplay(string $value): string
  {
    return $value !== '' ? $value : '-';
  }

  private function pendingContractUrl(string $value): string
  {
    $value = trim($value);
  ...
  private function renderPendingContractParty(string $role, string $badge, array $record, array $nameKeys, array $emailKeys, array $phoneKeys): string
  {
    $name = $this->pendingContractDisplay($this->pendingFirstRecordValue($record, $nameKeys));
    $email = $this->pendingContractDisplay($this->pendingFirstRecordValue($record, $emailKeys));
    $phone = $this->pendingContractDisplay($this->pendingFirstRecordValue($record, $phoneKeys));
  ...
  private function renderPendingContractMetric(string $label, string $value, string $help): string
  {
    return '<div class="scm-contract-metric"><span>' . esc_html($label) . '</span><strong>' . esc_html($this->pendingContractDisplay($value)) . '</strong>' . ($help !== '' ? '<small>' . esc_html($help) . '</small>' : '') . '</div>';
  }

  private function renderPendingContractTrace(string $label, string $value): string
  {
    if ($value === '') {
      return '';
    }
  ...
  private function pendingContractMoney(string $value): string
  {
    $value = trim($value);
    if ($value === '') {
      return '';
  ...
  private function pendingContractPercent(string $value): string
  {
    $value = trim($value);
    if ($value === '') {
      return '';
  ...
  private function renderPendingContractActions(array $buttons): string
  {
    if (empty($buttons)) {
      return '';
    }
