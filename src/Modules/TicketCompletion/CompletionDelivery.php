...
final class CompletionDelivery
{
  ...
  {
    return self::configuredTemplate('SCM_ACTA_WHATSAPP_OTP_TEMPLATE');
  }

  private static function configuredTemplate(string $key): string
  {
  ...
  public static function enqueue(Database $db, string $to, string $subject, string $html, array $options): int
  {
    ...
    $template = match ($event) {
      ...
      'signature_invitation' => self::configuredTemplate('SCM_ACTA_WHATSAPP_INVITATION_TEMPLATE'),
      'signed_receipt' => self::configuredTemplate('SCM_ACTA_WHATSAPP_RECEIPT_TEMPLATE'),
      default => null,
    };
    ...
    if ($otp) {
      ...
      // Preserve existing delivery until each dedicated template is approved and configured.
      $parameters = [(string) $options['destination_name'], $message, 'SKC SuCasa Inmobiliaria'];
    }
    ...
    $language = $template !== '' ? (trim((string) getenv($languageKey)) ?: 'es_CO') : 'es_CO';
    $meta = $options['meta'] + [
      'source_module' => 'ticket-completion', 'dedupe_key' => $options['dedupe_key'],
      'template_name' => $template !== '' ? $template : 'scm_notificacion_general_v1',
