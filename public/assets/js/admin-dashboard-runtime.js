(function () {
  ...
  function initRoot(root) {
        ...
        .then(function (payload) {
          ...
          return window.Swal.fire({
            ...
            didOpen: function () {
              ...
              popup.addEventListener("click", function (event) {
                var createTicketBtn = event.target && event.target.closest
                  ? event.target.closest("[data-scm-dashboard-due-create-ticket]")
                  : null;
                if (!createTicketBtn || !popup.contains(createTicketBtn)) return;
                event.preventDefault();
                openAdminTicketModal(createTicketBtn);
              });
              popup.addEventListener("click", function (event) {
    ...
    function initAdminNotificationsPanel(forceReload) {
      ...
      function downloadImportReport() {
        ...
        var columns = [
          ...
          ["nit_pdf", "NIT PDF"],
          ["inmuebles_pdf", "Inmuebles nombre PDF"],
          ["pdf_texto_leido", "PDF leido"],
          ["pagos_pdf", "Pagos detectados PDF"],
    ...
    function closePublicServicesReviewModal(force) {
      ...
      var form = modal.querySelector("[data-public-services-review-form]");
      if (!force && form && form.getAttribute("aria-busy") === "true") return;
    ...
    function submitPublicServicesReviewForm(form) {
      ...
      if (submitButton) {
        ...
        submitButton.textContent = operation === "submit" ? "Guardando y generando actas..." : "Guardando configuración...";
      }
      ...
      fetch(ajaxUrl, { method: "POST", body: fd, credentials: "same-origin" })
        ...
        .then(function (json) {
          ...
          if (window.Swal && typeof window.Swal.fire === "function") {
            window.Swal.fire({
              icon: "success",
              title: operation === "submit" ? "Revisión registrada" : "Configuración guardada",
    ...
    function replaceSelectOptions(select, rows, valueKey, labelKey) {
      if (!select) return;
      var current = String(
        ((runtime.initialFilters || {})[select.name] || select.value || select.getAttribute("data-current-employee") || ""),
      );
    ...
    function maintenanceQuotePerturbationHtml(context) {
      ...
      var conditionsBox = '<div class="scm-maint-quote-conditions-box"><h5 class="scm-maint-quote-perturb-section-title">Condiciones y perturbación</h5><div class="scm-maint-quote-grid">' +
        ...
        '</div><input type="hidden" name="justificacion_perturbacion" value="' + escHtml(d.justificacion_perturbacion || "") + '"><textarea name="resumen_calculo_perturbacion" hidden>' + escHtml(d.resumen_calculo_perturbacion || "") + '</textarea></div>';
      return (
    ...
    function buildMaintenanceQuoteFormHtml(context) {
      ...
      return (
        ...
        '<section class="scm-maint-quote-section scm-maint-quote-offers"><h4>Ofertas de materiales</h4><div class="scm-maint-quote-grid"><label class="scm-cotizacion-dialog-field"><span>Subir mejores ofertas adicionales</span><input type="file" name="mejor_oferta[]" accept="image/jpeg,image/png,image/webp" multiple><small>Opcional. Solo usa este campo si tienes imágenes externas. Actuales: ' + escHtml(String((media.mejor_oferta || []).length)) + '</small></label><label class="scm-cotizacion-dialog-field"><span>Subir otras ofertas adicionales</span><input type="file" name="otras_oferta[]" accept="image/jpeg,image/png,image/webp" multiple><small>Opcional. Actuales: ' + escHtml(String((media.otras_oferta || []).length)) + '</small></label></div><div class="scm-maint-quote-auto-offers"><strong>Mejores ofertas generadas por el sistema</strong><div data-material-generated-offer-attachments><p>No has generado imágenes desde la tabla de materiales.</p></div></div><p class="scm-maint-quote-help">Las ofertas generadas desde la tabla no aparecen dentro del selector de archivos por seguridad del navegador, pero se guardarán automáticamente como mejores ofertas. También puedes subir imágenes adicionales si ya las tienes.</p></section>' +
        ...
        maintenanceQuotePerturbationHtml(context) +
        '<section class="scm-maint-quote-section scm-maint-quote-bottom-section"><h4>Observaciones</h4><label class="scm-cotizacion-dialog-field is-wide"><span>Observaciones</span><textarea name="observaciones" rows="4">' + escHtml(d.observaciones || "") + '</textarea></label></section>' +
        '</form>'
      );
    }
