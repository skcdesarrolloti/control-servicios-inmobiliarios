...
$seedTicket(9);
$deleteInput = array_replace($input, ['items' => [['damage' => 'Acta de prueba', 'solution' => 'Sin fotos para limpieza aislada']]]);
$deletable = $service->create(9, $deleteInput, $actor);
$service->archive((int) $deletable['act_id'], 'Acta de prueba para eliminar', $actor);
$deletableAct = $repo->act((int) $deletable['act_id']);
$deleteTable = (new View())->dashboardTable([$deletableAct + ['_payload' => $service->payload($deletableAct)]], $service);
$assert(str_contains($deleteTable, 'data-acta-delete="' . $deletable['act_id'] . '"') && str_contains($deleteTable, '>Eliminar<'), 'archived or cancelled acts expose delete action in dashboard');
...
$editPanel = (new View())->panel($service->context(12), $service, (int) $updatedAct['id'], false);
$assert(str_contains($editPanel, 'data-acta-operation="update"') && str_contains($editPanel, 'Daño corregido') && str_contains($editPanel, 'Guardar cambios y reenviar firma') && !str_contains($editPanel, 'Acta enviada a bandeja'), 'pending act edit form is prefilled without showing the history card in direct mode');
$notifications = [];
...
$pendingPanel = (new View())->panel($service->context(1), $service);
$pendingDashboardTable = (new View())->dashboardTable([$act + ['_payload' => $service->payload($act)]], $service, [], true);
$assert(!str_contains($pendingPanel, '&format=pdf') && !str_contains($pendingDashboardTable, '&format=pdf'), 'pending acts do not expose PDF download actions');
$rejects(static fn() => $service->pdf($act), 'pending act PDF download is blocked');
...
$assert(!str_contains($signed['signed_pdf'], 'Firma electrónica; no corresponde') && !str_contains($publicHtml, 'No certifica la identidad documental'), 'act signature output omits the internal legal disclaimer');
...
$signedDeletableToken = $service->token($signedDeletableAct);
