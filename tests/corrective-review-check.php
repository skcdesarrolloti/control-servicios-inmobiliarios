...
$checks = [
  ...
  'trait tolerates stale property contract when explicit property id matches' => is_string($trait) && str_contains($trait, '$propertyMatchesExplicitContext = false') && str_contains($trait, '$propertyMatchesExplicitContext = true') && str_contains($trait, 'if (!$propertyMatchesExplicitContext && $contractId !=='),
