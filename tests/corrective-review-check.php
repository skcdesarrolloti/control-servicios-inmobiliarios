...
$checks = [
  ...
  'trait does not block read or edit panel with historical context mismatch' => is_string($trait) && str_contains($trait, 'No debe') && str_contains($trait, 'bloquear el editor') && substr_count($trait, 'correctiveReviewValidateContext($ticket, $contract, $property)') === 1,
  'trait validates property identity without mixing web and data ids' => is_string($trait) && str_contains($trait, '$propertyMatchesCaseContext = false') && str_contains($trait, "\$property['codigo'] ?? ''") && str_contains($trait, "\$ticket['id_inmueble'] ?? ''") && str_contains($trait, 'if (!$propertyMatchesCaseContext && $contractId !=='),
